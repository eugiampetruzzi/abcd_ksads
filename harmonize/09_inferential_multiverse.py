...
CONSTRUCTS = [("suicidality", "Suicidality"), ("eating", "Eating disorders"),
...
def load_culture_env():
    ...
    sc = [c for c in st.columns if c.startswith("screen") and ("_wkdy_y" in c or "_wknd_y" in c)]
    st["screentime"] = st[sc].apply(pd.to_numeric, errors="coerce").sum(axis=1, min_count=1)
    ...
    out = (st[["src_subject_id", "screentime"]]
           .merge(fes, on="src_subject_id", how="outer")
           .merge(dm, on="src_subject_id", how="outer")
           .rename(columns={"fes_y_ss_fc": "fam_conflict", "dim_y_ss_mean": "discrimination"}))
    out["participant_id"] = _nda_to_sub(out.src_subject_id)
    return out.set_index("participant_id")[["screentime", "fam_conflict", "discrimination"]]
...
def load_imaging():
    ...
    cols = ["src_subject_id", "eventname",
            "rsfmri_c_ngd_dt_ngd_dt", "rsfmri_c_ngd_sa_ngd_sa", "rsfmri_c_ngd_fo_ngd_fo",
            "rsfmri_c_ngd_dt_ngd_sa", "rsfmri_c_ngd_dt_ngd_fo", "rsfmri_c_ngd_sa_ngd_fo"]
    d = pd.read_csv(f, usecols=cols)
    d = d[d.eventname == BASELINE_EVENT].rename(columns={
    ...
    return d.set_index("participant_id")[["fc_dmn_within", "fc_sal_within", "fc_fpn_within",
                                          "fc_dmn_salience", "fc_dmn_fpn", "fc_sal_fpn"]]


FC_COLS = ["fc_dmn_within", "fc_sal_within", "fc_fpn_within",
           "fc_dmn_salience", "fc_dmn_fpn", "fc_sal_fpn"]
...
def load_qc_nuisance():
    ...
    cov = pd.read_excel(os.path.join(BASE, "5_covariates_extended.xlsx"))[
        ["sub_ID", "study_site_baseline", "scanner_manufacturer_baseline", "family_id",
         "rest_mean_FD_baseline", "rest_total_frames_post_scrubbing_baseline"]].rename(
        columns={"sub_ID": "participant_id", "study_site_baseline": "site",
                 "scanner_manufacturer_baseline": "scanner", "rest_mean_FD_baseline": "mean_fd",
                 "rest_total_frames_post_scrubbing_baseline": "frames_kept"})
    inc = pd.read_csv(os.path.join(BOX, "imaging", "mri_y_qc_incl.csv"),
    ...
    cov = cov.merge(inc[["participant_id", "imgincl_rsfmri_include"]], on="participant_id", how="left")
...
def load_predictors():
    ela = pd.read_excel(os.path.join(BASE, "4_ELA_final.xlsx"))[["sub_ID", "interview_age", "sex"]]
    cov = pd.read_excel(os.path.join(BASE, "5_covariates_extended.xlsx"))[["sub_ID", "Race", "Income"]]
    d = ela.merge(cov, on="sub_ID", how="outer").rename(columns={"sub_ID": "participant_id"})
    ...
    d = d.join(load_qc_nuisance()[["site", "scanner", "family_id", "mean_fd", "fc_qc_pass"]], how="left")
    ...
    d["mean_fd_z"] = (pd.to_numeric(d.mean_fd, errors="coerce") - fdq.mean()) / fdq.std()
...
def fit_adj(d, focal, imaging=False):
    ...
    try:
        ...
        if imaging:
            X = X.join(pd.get_dummies(d.scanner, prefix="scn", drop_first=True).astype(float))
        ...
        for t in focal:
            orr = float(np.exp(res.params[t])); p = float(res.pvalues[t])
...
def main():
        ...
        for inf in INFORMANTS:
            for status in STATUSES:
                for subthr in THRESH:
                    stat = M6.construct_status(cache, con, status, inf, subthr, "phobia_in")
                    if stat is None or len(stat) == 0:
                        continue
                    y = stat.map({"positive": 1, "administered_negative": 0,
                                  "not_administered": np.nan})
                    df = P.join(y.rename("y"), how="inner").dropna(subset=["y"])
                    spec = dict(construct=con, construct_label=conlab, informant=inf,
                                status=status, threshold="with_subthreshold" if subthr else "full")
                    ...
                    for col, lab in [("screentime_z", "Screen time (per SD)"),
                    ...
                    for col, lab in [("fc_dmn_within_z", "DMN within-network FC (per SD)"),
                    ...
                    if enough(d.y):
                        dummies = pd.get_dummies(d.Race).reindex(columns=RACE_LVES, fill_value=0).astype(float)
                    ...
                    for lab, (orr, p) in out.items():
                        bucket = ("Sex" if lab.startswith("Female") else
                                  "Income" if lab.startswith("Income") else
                                  "Race/ethnicity" if lab.startswith("Race") else
                                  "Neuroimaging" if "FC" in lab else
                                  "Culture/environment")
                        rows.append({**spec, "bucket": bucket, "predictor": lab, "OR": orr, "p": p})
    ...
    summ = []
    for (bucket, con, conlab, pred), sub in res.groupby(["bucket", "construct", "construct_label", "predictor"]):
        sub = sub.dropna(subset=["OR"])
        if sub.empty:
            continue
        flips = (sub.OR.min() < 1) and (sub.OR.max() > 1)
        row = {"bucket": bucket, "construct": con, "construct_label": conlab, "predictor": pred,
               "n_specs": len(sub), "OR_min": sub.OR.min(), "OR_max": sub.OR.max(),
               "pct_sig": 100 * sub.sig.mean(), "sign_flip": flips,
               "any_sig": sub.sig.any(), "all_sig": sub.sig.all()}
        row.update(eta2(sub))
        summ.append(row)
    S = pd.DataFrame(summ)
    ...
    n_pairs = len(S)
    print(f"Fit {len(res.dropna(subset=['OR']))} specifications across {n_pairs} predictor x construct pairs.")

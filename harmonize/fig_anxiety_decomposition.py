#!/usr/bin/env python3
"""Anxiety construct = a curation choice. Baseline (ses-00A) present-diagnosis
prevalence of each parent KSADS anxiety sub-disorder, and the "any anxiety"
prevalence as sub-disorders are added cumulatively. Specific phobia alone drives
most of the construct, so including vs excluding it swings anxiety several-fold.

Output: derivatives/fig_anxiety_decomposition.png + anxiety_decomposition.csv
"""
import os
...
SUBS = [("gad", "GAD"), ("sepanx", "Separation"), ("socanx", "Social"),
...
def main():
    ...
    for c in ["session_id", "module", "informant", "status_layer", "resolved"]:
        r[c] = r[c].astype(str)
    base = r[(r.session_id == "ses-00A") & (r.informant == "parent")
    ...
    for mod, lab in SUBS:
        ...
        rows.append({"sub": lab, "n_pos": len(pos), "n_assessed": len(assessed),
    ...
    any_with = cum[-1]
    any_without_phobia = 100 * len(set().union(
        *[pos_sets[m] for m, _ in SUBS if m != "phobia"])) / len(assessed_all)
    dec_out = pd.concat([dec, pd.DataFrame([
        {"sub": "ANY (without phobia)", "n_pos": np.nan, "n_assessed": len(assessed_all),
         "prevalence_pct": round(any_without_phobia, 2)},
        {"sub": "ANY (with phobia)", "n_pos": np.nan, "n_assessed": len(assessed_all),
         "prevalence_pct": round(any_with, 2)}])], ignore_index=True)
    ...
    ax2.text(1, any_without_phobia + 0.3, f"without phobia = {any_without_phobia:.1f}%",
             fontsize=8, color="#555555")
    fig.tight_layout()
    fig.savefig(os.path.join(DERIV, "fig_anxiety_decomposition.png"),
    ...
    print(f"-> including specific phobia changes the anxiety construct "

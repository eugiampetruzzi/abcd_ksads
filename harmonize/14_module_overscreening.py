#!/usr/bin/env python3
"""Module-level baseline prevalence, CORE CRITERIA ONLY (subthreshold excluded).

Every rate here is computed from full-criteria diagnoses only -- "other specified"
and "unspecified" variants are dropped -- so no number is inflated by subthreshold
inclusion. Reports (a) baseline parent present prevalence per module, flagging the
modules that screen well above childhood epidemiology, and (b) the depression
current-vs-past breakdown that drives the ever-met option.

Output: derivatives/module_overscreening.csv, status_depression_breakdown.csv
"""

#!/usr/bin/env python3
"""Technical validation of the harmonized dataset (three checks).

1. Correctness: positives in the resolved layer reproduce the raw ABCD value-1
   counts exactly (harmonization relabels administration state; it never changes
   a diagnosis).
2. Face validity: default-configuration (current, parent) baseline prevalence vs
   US national current-diagnosed rates (CDC Children's Mental Health, NSCH
   2022-2023; ages 3-17).
3. Parent-youth concordance: Cohen's kappa for the categories both informants
   assess at baseline (validates the informant feature; low agreement expected).

Outputs: derivatives/validation_correctness.csv, validation_facevalidity.csv,
         validation_concordance.csv, technical_validation_report.txt
"""
import csv

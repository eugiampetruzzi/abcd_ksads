...
def main():
    ...
    def age_date(pref, mod="dep"):
        ...
        cols = [
            "participant_id",
            "session_id",
            f"{pref}_ksads__{mod}_age",
            f"{pref}_ksads__{mod}_dtt",
        ]
    ...
    res = (
        pd.read_parquet(
            os.path.join(
                ...
                "harmonize",
                "derivatives",
                "ksads_resolved_versioned.parquet",
            )
        )
        if False
        else None
    ...
    rl = pd.read_csv(
        os.path.join(PHENO, "ksads_diagnosis_resolved.tsv.gz"), sep="\t", dtype=str
    )
    rl.to_csv(
        os.path.join(CSV, "ksads_diagnosis_resolved.csv.gz"),
        index=False,
        compression="gzip",
    )
    ...
    age_cov = (
        pd.read_csv(
            os.path.join(CSV, "ksads_categories_current.csv"), usecols=["interview_age"]
        )
        .interview_age.notna()
        .mean()
    )

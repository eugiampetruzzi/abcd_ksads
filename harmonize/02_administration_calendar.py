...
ADMIN_THRESHOLD = (
    0.5  # fraction of (administered / non-no_record) to call "administered"
)
...
def main():
    ...
    for (inf, mod), sub in g.groupby(["informant", "module"]):
        ...
        if not admin_any:
            ...
            if (
                admin_even and admin_even[-1] != "ses-06A"
            ):  # stops before the biennial endpoint
    ...
    # readable grid
    grid = g.assign(cell=g.status.map(GRID)).pivot_table(

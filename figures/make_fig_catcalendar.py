...
SES=["ses-00A","ses-01A","ses-02A","ses-03A","ses-04A","ses-05A","ses-06A","ses-07A"]

# DSM category -> (N, diagnosis list, parent modules, youth modules)
CATS=[
 ("Anxiety",6,"GAD; separation anxiety; social anxiety; panic; agoraphobia; specific phobia",
  ["gad","sepanx","socanx","panic","agor","phobia"],["gad","socanx","panic"]),
 ("Eating",3,"Anorexia nervosa; bulimia nervosa; binge-eating disorder",["ed"],["ed"]),
 ("Psychosis",3,"Schizophrenia; schizoaffective; schizophreniform",["psych"],[]),
 ("Tic",3,"Tourette's; persistent tic; provisional tic",["tic"],[]),
 ("Depression",2,"Major depressive disorder; persistent depressive disorder",["dep"],["dep"]),
...
x0=7.1; cw=0.62; gap=0.06; hh=0.34
...
for i,(cat,N,dx,pmods,ymods) in enumerate(CATS):
    ...
    for j,ses in enumerate(SES):
        xc=x0+j*(cw+gap)
        ax.add_patch(Rectangle((xc,y),cw,hh,facecolor=color(pmods,"parent",ses),edgecolor="white",lw=0.8))   # parent top
        ax.add_patch(Rectangle((xc,y-hh),cw,hh,facecolor=color(ymods,"youth",ses),edgecolor="white",lw=0.8)) # youth bottom
for j,ses in enumerate(SES):
    ax.text(x0+j*(cw+gap)+cw/2,n-0.32,ses.replace("ses-",""),ha="center",va="bottom",fontsize=8.5)
# explicit parent/youth labels to the right of the first (Anxiety) row
xr=x0+8*(cw+gap)+0.12
ax.annotate("Parent",(xr-0.06,n-1+hh/2),xytext=(xr+0.18,n-1+hh/2),va="center",ha="left",fontsize=8.5,color="#444",
            arrowprops=dict(arrowstyle="-",color="#999",lw=0.8))
ax.annotate("Youth",(xr-0.06,n-1-hh/2),xytext=(xr+0.18,n-1-hh/2),va="center",ha="left",fontsize=8.5,color="#444",
            arrowprops=dict(arrowstyle="-",color="#999",lw=0.8))
ax.text(-0.05,n-0.05,"DSM category",ha="left",va="bottom",fontweight="bold",fontsize=9.5)
ax.text(cN,n-0.05,"N",ha="center",va="bottom",fontweight="bold",fontsize=9.5)
ax.text(cDx,n-0.05,"Diagnoses",ha="left",va="bottom",fontweight="bold",fontsize=9.5)
ax.text(x0+3*(cw+gap)+cw/2,n+0.78,"Session",ha="center",va="bottom",fontweight="bold",fontsize=9.5)
# version boundary between ses-02A and ses-03A
xb=x0+3*(cw+gap)-gap/2
ax.plot([xb,xb],[-0.6,n+0.28],color="#444",lw=1.2,ls=(0,(4,3)))
ax.text(x0+1*(cw+gap),n+0.32,"KSADS-COMP 1.0",ha="center",va="bottom",fontsize=8,color="#444")
ax.text(x0+5.4*(cw+gap),n+0.32,"KSADS-COMP 2.0",ha="center",va="bottom",fontsize=8,color="#444")
# legend (color states)
leg=[Line2D([0],[0],marker="s",ls="",mfc=BLUE,mec="white",ms=11,label="Administered"),
     Line2D([0],[0],marker="s",ls="",mfc=RED,mec="white",ms=11,label="Not administered"),
     Line2D([0],[0],marker="s",ls="",mfc=GREY,mec="white",ms=11,label="Not administered to youth")]
ax.legend(handles=leg,loc="lower left",bbox_to_anchor=(0.0,-0.12),frameon=False,

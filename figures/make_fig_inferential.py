...
for i,row in h.iterrows():
    ...
    axa.scatter(i,row.OR,s=70,color=(c if row.sig else "white"),edgecolor=c,linewidth=1.4,zorder=3)
axa.axhline(1.0,color=GREY,lw=1.0,ls=(0,(5,4)),zorder=1)
axa.set_xticks([]); axa.set_xlabel("Operationalizations, ordered by odds ratio")
axa.set_ylabel("Odds ratio: female vs male, depression")
axa.set_xlim(-0.7,len(h)-0.3)
lg=[Line2D([0],[0],marker="o",ls="",mfc=BLUE,mec=BLUE,label="Significant (p<.05)"),
...
axa.text(0.02,0.02,"OR>1: girls higher\nOR<1: girls lower",transform=axa.transAxes,
...
def lab(r):
    p=r.predictor.replace("Female (vs male)","Female").replace(" (per SD)","").replace("Race: ","")
    return f"{p} - {r.construct_label}"
S=S.copy(); S["label"]=S.apply(lab,axis=1)
...
for bk in ["Sex","Income","Race/ethnicity","Culture/environment","Neuroimaging"]:
    ...
    for _,row in g.iterrows():
        ...
        axb.plot([row.OR_min,row.OR_max],[y,y],color=c,lw=2.4,alpha=0.85,solid_capstyle="round",zorder=2)
        axb.scatter(row.med,y,s=16,color=c,zorder=3)
        yt.append(y); ylab.append(row.label); y+=1
    axb.text(0.205,y-len(g)/2-0.5,bk,rotation=90,va="center",ha="center",fontsize=9,
             fontweight="bold",color="#333")
    seps.append(y-0.5); y+=0.8
axb.axvline(1.0,color="#333",lw=1.0,zorder=1)
for s in seps[:-1]:
    axb.axhline(s,color="#dddddd",lw=0.8,zorder=0)
axb.set_yticks(yt); axb.set_yticklabels(ylab,fontsize=6.3)
axb.set_xscale("log"); axb.set_xlabel("Odds ratio across operationalizations (log scale)")
axb.set_ylim(-0.8,y-0.8)
axb.set_xticks([0.25,0.5,1,2,4]); axb.set_xticklabels(["0.25","0.5","1","2","4"])
axb.set_xlim(0.2,5.5)
lg2=[Line2D([0],[0],color=RED,lw=2.4,label="Sign flips (crosses OR=1)"),

...
def fig_trajectories():
    ...
    ncol=3; nrow=int(np.ceil(len(cats)/ncol))
    ...
    for ax,cat in zip(axes.flat,cats):
        for inf,c in (("parent",BLUE),("youth",RED)):
            d=PREV[(PREV.category==cat)&(PREV.informant==inf)].dropna(subset=["prevalence_pct"])
            d=d[d.n_denominator>0]
            x=[XV[s] for s in d.session]; y=d.prevalence_pct.values
        ...
        ax.set_xticks(range(len(SES))); ax.set_xticklabels(XLAB)
        ax.set_xlim(-0.3,len(SES)-0.7)
    for ax in axes.flat[len(cats):]: ax.axis("off")
    for ax in axes[-1]: ax.set_xlabel("Years from baseline")
    for ax in axes[:,0]: ax.set_ylabel("Prevalence (%)")
    hl=[Line2D([0],[0],color=BLUE,marker="o",mec="white",label="Caregiver"),
...
def fig_dumbbell():
    ...
    for cat in ORDER:
        for ses in reversed(SES):
            ...
            dp=d[d.informant=="parent"]; dy=d[d.informant=="youth"]
            if len(dp) and len(dy) and dp.n_denominator.iloc[0]>0 and dy.n_denominator.iloc[0]>0:
                k=CONC[(CONC.category==cat)&(CONC.session==ses)]
                rows.append({"category":cat,"session":ses,"parent":dp.prevalence_pct.iloc[0],
                             "youth":dy.prevalence_pct.iloc[0],
                             "kappa":k.kappa.iloc[0] if len(k) else np.nan}); break
    D=pd.DataFrame(rows).iloc[::-1].reset_index(drop=True)
    fig,ax=plt.subplots(figsize=(8.6,5.0))
    for i,r in D.iterrows():
        ax.plot([r.parent,r.youth],[i,i],color=GREY,lw=1.6,zorder=1)
        ax.scatter(r.parent,i,s=85,color=BLUE,edgecolor="white",linewidth=0.7,zorder=3)
        ax.scatter(r.youth,i,s=85,color=RED,edgecolor="white",linewidth=0.7,zorder=3)
        xr=max(r.parent,r.youth)
        ax.annotate(f"κ={r.kappa:.2f}",(xr,i),xytext=(8,0),textcoords="offset points",
                    va="center",ha="left",fontsize=8,color="#444")
    ax.set_yticks(range(len(D))); ax.set_yticklabels([f"{r.category}" for _,r in D.iterrows()])
    ax.set_xlabel("Prevalence (%)"); ax.set_ylim(-0.6,len(D)-0.4)
    hl=[Line2D([0],[0],marker="o",ls="",mfc=BLUE,mec="white",label="Caregiver"),
...
def fig_diverging():
    ...
    d=d.set_index("category").reindex([c for c in ORDER if c in d.category.values]).dropna(how="all").reset_index()
    d["p_share"]=100*d.parent_only/d.union_pos; d["y_share"]=100*d.youth_only/d.union_pos
...
if __name__=="__main__":
    fig_trajectories(); fig_dumbbell(); fig_diverging()